...
// Lazy load 3D canvas ONLY on desktop devices for max mobile performance
const Server3DCanvas = lazy(() => import('./3d/Server3DCanvas'));
...
export const Terminal: React.FC = () => {
  ...
  // Mobile detection for disabling 3D Canvas & heavy tilt calculation
  ...
  const springRotateX = useSpring(rawRotateX, { stiffness: 140, damping: 18 });
  const springRotateY = useSpring(rawRotateY, { stiffness: 140, damping: 18 });

  // Disable 3D tilt entirely on mobile devices
  const rotateX = isMobile ? 0 : springRotateX;
  const rotateY = isMobile ? 0 : springRotateY;
  ...
  // Detect Device, OS & Hardware Specs
  useEffect(() => {
  ...
  return (
    <div
      ...
      className="relative w-full max-w-md sm:max-w-lg lg:max-w-xl mx-auto perspective-1000 py-2 transform-gpu"
    >
      {/* 3D Tilt Wrapper */}
      <motion.div
        style={{
        ...
        className="relative w-full rounded-2xl p-1 bg-gradient-to-b from-white/15 via-white/5 to-cyan-500/20 backdrop-blur-md md:backdrop-blur-2xl shadow-[0_20px_50px_-15px_rgba(6,182,212,0.25)] border border-white/10"
      >
        {/* Render 3D Canvas ONLY on Desktop; Lightweight CSS Graphic on Mobile */}
        {!isMobile ? (
          ...
          <div className="absolute inset-0 pointer-events-none rounded-2xl bg-gradient-to-tr from-cyan-500/10 via-purple-500/5 to-transparent flex items-center justify-center opacity-30">
            <Server className="w-24 h-24 text-cyan-400/20 animate-pulse" />
          </div>
        )}

        {/* Floating Real-Time Status Badges */}
        {/* Pill 1: Visitor Device (Top Left) */}
        <div
          className="absolute -top-4 -left-2 sm:-left-4 z-30 flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-950/90 border border-emerald-500/40 backdrop-blur-md shadow-lg text-[11px] font-mono text-emerald-300"
        ...
        {/* Pill 2: Network (Top Right) */}
        <div
          className="absolute -top-4 -right-2 sm:-right-4 z-30 flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-950/90 border border-cyan-500/40 backdrop-blur-md shadow-lg text-[11px] font-mono text-cyan-300"
          style={{ transform: isMobile ? 'none' : 'translateZ(35px)' }}
        >
          <Wifi className="w-3 h-3 text-cyan-400 animate-pulse" />
          <span className="font-medium">Network: {isOnline ? 'Online' : 'Offline'}</span>
        </div>

        {/* Pill 3: Session Uptime (Bottom Left) */}
        <div
          className="absolute -bottom-4 -left-2 sm:-left-4 z-30 flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-950/90 border border-purple-500/40 backdrop-blur-md shadow-lg text-[11px] font-mono text-purple-300"
          style={{ transform: isMobile ? 'none' : 'translateZ(25px)' }}
        >
          <Clock className="w-3 h-3 text-purple-400 animate-spin [animation-duration:10s]" />
          <span className="font-medium">Uptime: {formatUptime(sessionSeconds)}</span>
        </div>

        {/* Main Terminal Window */}
        <div className="relative z-20 rounded-xl bg-zinc-950/95 md:bg-zinc-950/90 backdrop-blur-md md:backdrop-blur-2xl border border-white/10 overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center justify-between px-3.5 py-2.5 bg-zinc-900/90 border-b border-white/10 select-none">
            ...
            <div className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full bg-red-500/80 border border-red-400/40 flex items-center justify-center group cursor-pointer">
                <span className="text-[7px] text-red-950 opacity-0 group-hover:opacity-100 font-bold">×</span>
              </div>
              <div className="w-2.5 h-2.5 rounded-full bg-amber-500/80 border border-amber-400/40 flex items-center justify-center group cursor-pointer">
                <span className="text-[7px] text-amber-950 opacity-0 group-hover:opacity-100 font-bold">-</span>
              </div>
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500/80 border border-emerald-400/40 flex items-center justify-center group cursor-pointer">
                <span className="text-[7px] text-emerald-950 opacity-0 group-hover:opacity-100 font-bold">+</span>
              </div>
            </div>

            {/* Terminal Title */}
            <div className="flex items-center gap-1.5 text-[11px] sm:text-xs font-mono text-zinc-300 font-medium">
          ...
          >
            <div className="text-zinc-500 text-[10px] sm:text-[11px] pb-2 border-b border-zinc-800/60 mb-2 flex items-center justify-between">
          ...
          {/* Footer Specs & Action */}
          <div className="px-3.5 py-2 bg-zinc-900/80 border-t border-white/5 flex items-center justify-between text-[10px] sm:text-[11px] font-mono text-zinc-400">
